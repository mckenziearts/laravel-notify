@if ($notify && $notify['model'] === 'toast')
    <div
        x-data="{ show: @if($notify['model'] === 'toast') true @else false @endif }"
        x-show="show"
        ...
        class="max-w-sm w-full @if(config('notify.theme') === 'light') bg-white @else bg-gray-800 @endif shadow-lg rounded-lg pointer-events-auto border-l-4 @if($notify['type']  === 'success') border-green-600 @endif @if($notify['type']  === 'warning') border-yellow-400 @endif @if($notify['type']  === 'info') border-blue-600 @endif @if($notify['type']  === 'error') border-red-600 @endif"
    >
            ...
            <div class="p-4">
                <div class="flex items-start">
                    ...
                    <div class="ml-4 w-0 flex-1">
                        <p class="text-base leading-5 font-medium capitalize @if($notify['type']  === 'success') text-green-600 @endif @if($notify['type']  === 'warning') text-yellow-400 @endif @if($notify['type']  === 'info') text-blue-600 @endif @if($notify['type']  === 'error') text-red-600 @endif">
                            {{ $notify['title']  ?? $notify['type']  }}
                        </p>
